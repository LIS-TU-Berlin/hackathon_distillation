...
class Policy(nn.Module):
    ...
    def __init__(
        ...
        self.expected_image_keys = [k for k in self.policy.config.network.input_shapes if k.startswith("obs.img")]
    ...
    def select_action(self, batch: dict[str, Tensor]) -> np.ndarray:
        if len(self.expected_image_keys) > 0:
            batch = dict(batch)
            batch["obs.img"] = th.stack([batch[k] for k in self.expected_image_keys], dim=-4)
