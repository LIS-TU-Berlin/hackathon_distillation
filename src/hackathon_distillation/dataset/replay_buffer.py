...
def h5_to_replaybuffer(
    ...
    If zarr_out_path is given -> create an on-disk zarr buffer. Otherwise -> in-memory numpy buffer.
    """
    # 1) create empty buffer (zarr or numpy)
    if zarr_out_path is None:
        if verbose: print("Building in-memory numpy replay buffer…")
        rb = ReplayBuffer.create_empty_numpy()
        resolved_compressor = None
    else:
        if verbose: print(f"Building on-disk zarr at: {zarr_out_path}")
        store = zarr.DirectoryStore(os.path.expanduser(zarr_out_path))
        root = zarr.group(store=store, overwrite=True)
        rb = ReplayBuffer.create_empty_zarr(root=root)
        resolved_compressor = ReplayBuffer.resolve_compressor(compressor)

    # 2) find episode groups like epi0000, epi0001, …
    ...
    try:
        manifest = h5_reader.read_dict('manifest')
        rb.update_meta({
            'tau_step': np.array(manifest['tau_step']),
            'num_episodes': np.array(manifest['num_episodes']),
        })
    except Exception:
        pass  # manifest optional

    # 4) stream episodes and append
    for epi in tqdm(epi_names, total=len(epi_names), desc="Processing episodes"):
        episode = {}
        for k in keys:
            arr = np.array(h5_reader.read(f"{epi}/{k}"))
            if T is None:
                T = len(arr)
            else:
                assert len(arr) == T, f"{epi}: {k} length mismatch (got {len(arr)} vs {T})"
            episode[k] = arr

        # pass per-key chunk/codec hints only for first time each key appears
        if rb.backend == 'zarr':
            # prepare one-time hints on first creation of each dataset
            per_key_chunks = {}
            per_key_comp = {}
            for k, val in episode.items():
                # choose chunks for time dimension only if not provided
                if chunks and k in chunks:
                    per_key_chunks[k] = chunks[k]
                else:
                    # good default: ~2MB target chunk size in time dimension
                    per_key_chunks[k] = get_optimal_chunks(val.shape, val.dtype, target_chunk_bytes=2e6)
                per_key_comp[k] = resolved_compressor

            rb.add_episode(episode, chunks=per_key_chunks, compressors=per_key_comp)
        else:
            rb.add_episode(episode)

    if verbose:
        print(f"Done. n_episodes={rb.n_episodes}, n_steps={rb.n_steps}, T={T}")
        if rb.backend == 'zarr':
            print(f"Chunk size (time): {rb.chunk_size}")
...
if __name__ == "__main__":
    ...
    print("episode lengths:", rb.episode_lengths)
    e0 = rb.get_episode(0)          # dict with arrays sliced to ep0
    some = rb.get_steps_slice(100, 120)