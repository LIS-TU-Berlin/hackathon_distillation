...
class Robot:
    ...
    def replay(self):
        ...
        for i, episode in enumerate(reader.fil.keys()):
            ...
            data_ee_action = reader.read(f"{episode}/ee_action")

            for action in data_ee_action:
                ...
                camera_relative_ball_pos = action.copy()
                ...
                q_target, ret = self.IK(camera_relative_ball_pos)
                if ret.feasible:
            ...
            if i == self.args.ep - 1:
                break  # only run some episodes for demo
